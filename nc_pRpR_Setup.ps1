...
if (($ncVer.Contains(".") -ne $true)) {$ncVer = $ncVer + ".0"}
...
$ncConfigFolderName = "\Config\"
$ncConfigFileName = "cfg.ini"
$ncConfigFolderPath = $env:USERPROFILE + "\AppData\Roaming\Nanosoft\nanoCAD x64 " + $ncVer + $ncConfigFolderName
$fullConfigFilePath = $ncConfigFolderPath + $ncConfigFileName

# Полный путь к настроенному конфиг-файлу (для локального установщика путь можно изменить на относительный - в каталог с установщиком)
$sourceFullFilePath = "\\SERVER\_Шаблоны\nCAD Templates\Configs\Roaming_Nanosoft_nanoCAD_Config\" + $ncConfigFileName


if (Test-Path -Path $fullConfigFilePath)
{
...
Copy-Item -Path $sourceFullFilePath -Destination $fullConfigFilePath -Force # Копирование файла в папке назначения с заменой файла если он там уже есть
...
# -------------  Импорт reg-файла в реестр с настройками для нанокада (так же возможно выполнять импорт непосредственно в теле кода этого скрипта, но из reg-файла удобнее, но если права пользователя ограничены то винда запросит ввести пароль пользователя)
$regFileName = "nCAD_TemplateDocs+ToolPalette.reg" # Имя reg-файла с настройками nanoCAD
$regFilePath = "\\SERVER\_Шаблоны\nCAD Templates\Configs\Настройка палитр и шаблонов\" + $regFileName # Путь к каталогу с reg-файлом (так же можно заменить на относительный пкть например - $PSScriptRoot+"\RegFiles")
